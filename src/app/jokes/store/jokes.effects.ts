...
export class JokesEffects {
    getManyJokes$ = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(({ count }) =>
                this.jokesService.getManyJokes(count).pipe(
    ...
    getOneJoke$ = createEffect(() =>
        this.actions$.pipe(
            ofType(JokeActions.getOneJoke),
            mergeMap(() =>
                this.jokesService.getOneJoke().pipe(
    ...
    persistOneJoke$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType(JokeActions.getOneJokeSuccess),
                withLatestFrom(this.store.select(selectJokes)),
                map(([, jokesInState]) =>
                    this.jokeDataService.persistJokeData(
                        JOKES_KEY,
                        jokesInState.slice(0, 10)
                    )
                )
            ),
        { dispatch: false }
    ...
    persistManyJokes$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType(JokeActions.getJokesSuccess),
                map(({ jokes }) =>
                    this.jokeDataService.persistJokeData(JOKES_KEY, jokes)
                )
            ),
        { dispatch: false }
    )
    ...
    addToFavourites$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                map(({ jokeToAdd }) =>
                    this.jokeDataService.persistToFavourites(jokeToAdd)
                )
            ),
        { dispatch: false }
    )
        ...
        () =>
            this.actions$.pipe(
                ...
                map(([{ jokeId }, favJokes]) =>
                    this.jokeDataService.persistJokeData(
                        FAVOURITE_JOKES_KEY,
                        favJokes.filter((joke) => joke.id !== jokeId)
                    )
                )
            ),
        { dispatch: false }
